...
CREATE TABLE IF NOT EXISTS `surtidor`.`administrador` (
  ...
  `AD_otronombre` VARCHAR(20) NULL,
  `AD_primeapellido` VARCHAR(20) NOT NULL,
  `AD_segundoapellido` VARCHAR(20) NULL,
...
CREATE TABLE IF NOT EXISTS `surtidor`.`ClienteWeb` (
  ...
  `CLWEB_contrasena` VARCHAR(20) NOT NULL,
  ...
  `CLWEB_otronombre` VARCHAR(20) NULL,
  `CLWEB_primerapellido` VARCHAR(20) NOT NULL,
  `CLWEB_segundoapellido` VARCHAR(20) NULL,
  ...
  INDEX `fk_USUARIOWEB_CLIENTE1_idx` (`CL_ID` ASC),
  UNIQUE INDEX `CL_pregunta_UNIQUE` (`CLWEB_pregunta` ASC),
  CONSTRAINT `fk_USUARIOWEB_CLIENTE1`
    FOREIGN KEY (`CL_ID`)
    REFERENCES `surtidor`.`cliente` (`CL_ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = latin1;
...
CREATE TABLE IF NOT EXISTS `surtidor`.`producto` (
  ...
  `PR_foto` VARCHAR(45) NULL,
  `AD_ID` INT NOT NULL,
  PRIMARY KEY (`PR_ID`, `PR_referencia`, `AD_ID`),
...
CREATE TABLE IF NOT EXISTS `surtidor`.`proveedor` (
  ...
  `AD_ID` INT NOT NULL,
  PRIMARY KEY (`PRV_ID`, `PR_ID`, `AD_ID`),
